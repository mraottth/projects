...
index_order = df.groupby('county')['cases'].max().sort_values(ascending=True).index
...
for curr_month in list(np.sort(df['date'].unique())):
    ...
    df_curr['prev_month'] = df_curr.groupby('county')['new_cases'].shift(1).fillna(0)
    df_curr['prev_month2'] = df_curr.groupby('county')['new_cases'].shift(2).fillna(0)


    # Make summary stats table with 25th, mean, 75th percentiles
    lower_bound_cases = pd.DataFrame(df_curr.groupby(['county'])['new_cases'].quantile(0.25))
    upper_bound_cases = pd.DataFrame(df_curr.groupby(['county'])['new_cases'].quantile(0.75))
    stats_cases = pd.DataFrame(df_curr.groupby(['county'])['new_cases'].agg([np.max, np.min, np.median, np.mean]))
    current_cases = df_curr.query('date == @curr_month')[['county','new_cases']].set_index('county')
    ...
    df_stat = pd.merge(lower_bound_cases, upper_bound_cases, left_index=True, right_index=True, suffixes=('low','mean'))\
                .merge(current_cases, left_index=True, right_index=True)\
                .merge(prev_month, left_index=True, right_index=True)\
                .merge(stats_cases, left_index=True, right_index=True)\
                .merge(prev_month2, left_index=True, right_index=True)\

    df_stat = df_stat.rename(columns={
                    'new_caseslow':'25th Percentile',
                    'new_casesmean':'75th Percentile',
                    'new_cases':'Last Month',
                    'prev_month':'2 Months Ago',
                    'prev_month2':'3 Months Ago',
                    'mean':'Mean New Cases',
                    'median':'Median New Cases',
                    'amin':'Min New Cases',
                    'amax':'Max New Cases'
                    }
                ).reindex(index_order)   
    ...
    df_stat['last_month_to_med'] = (df_stat['Last Month'] - df_stat['Median New Cases']) / df_stat['Median New Cases']
    ...
    for ax in (a0, a1):
        ...
        # plt.scatter(
        #     df_stat.index,
        #     df_stat['Min New Cases'],
        #     marker='_', 
        #     # linestyle='dashed',
        #     label='Min Monthly Cases',
        #     color='g',
        #     s=400,
        #     alpha=0.2
        #     )
        ...
        a0.set_ylabel('Cases per 100k County Residents\n', fontsize=16)
        a0.set_xlabel(None)
        a0.set_title('Monthly COVID-19 Cases: Top 30 Counties Comparison \n Data Thru: ' + calendar.month_abbr[curr_month.month] + ' ' + str(curr_month.year), fontsize=24)    
        a1.set_title('King County\nDrilldown', fontsize=24)   
        a1.text(x=0.625, y=-165, s='King County\nDrilldown', color='teal', fontsize=14, rotation=90, ha='right', fontweight='bold')

        a0.legend(fontsize=14, loc=2) 
        
    # plt.legend(fontsize=14, bbox_to_anchor=(1.01,1), borderaxespad=0)
    # plt.legend(fontsize=14, loc=2)
    plt.savefig('/Users/mattroth/Desktop/Seattle IP Team/Charts/Full Pandemic History/Drilldown/Cases/cases' + str(curr_month)[0:10] + '_drilldown.jpeg', bbox_inches = "tight", dpi=150)
    plt.show()  
...
jpeg_dir = '/Users/mattroth/Desktop/Seattle IP Team/Charts/Full Pandemic History/Drilldown/Cases'
images = []
for file_name in sorted(os.listdir(jpeg_dir)):
    if file_name.endswith('.jpeg'):
        file_path = os.path.join(jpeg_dir, file_name)
        images.append(imageio.imread(file_path))
imageio.mimsave('/Users/mattroth/Desktop/Seattle IP Team/Charts/Full Pandemic History/Drilldown/Cases/cases_pandemic_history_drilldown.gif', images, duration = 1.25)